...
# ==========================================
# ==========================================
# 4. MAIN EXECUTION (Updated for 5-Fold CV)
# ==========================================
if __name__ == "__main__":
    ...
    if not files:
        ...
        print("\nStarting 5-Fold CV for CNN Baseline...")
        for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):
            ...
            model.fit(X_train, y_train, epochs=CONF['EPOCHS'], batch_size=CONF['BATCH_SIZE'], verbose=0)
            
            y_pred = (model.predict(X_test, verbose=0) > 0.5).astype(int)
            fold_accs.append(accuracy_score(y_test, y_pred))
            fold_f1s.append(f1_score(y_test, y_pred))
            
            print(f"Fold {fold+1} Accuracy: {fold_accs[-1]:.4f}")
